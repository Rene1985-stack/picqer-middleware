  ...
  async initializeUsersDatabase() {
    ...
    await pool.request().query(`
      IF NOT EXISTS (
        ...
        WHERE Name = N'email' AND Object_ID = Object_ID(N'Users')
      ) BEGIN
      ...
      IF NOT EXISTS (
        SELECT * FROM sys.columns 
        WHERE Name = N'last_sync_date' AND Object_ID = Object_ID(N'Users')
      ) BEGIN
        ALTER TABLE Users ADD last_sync_date DATETIME;
      END;

      IF NOT EXISTS (
        SELECT * FROM SyncStatus WHERE entity_name = 'users'
      ) BEGIN
        INSERT INTO SyncStatus (entity_name, entity_type, last_sync_date, last_sync_count, total_count)
        VALUES ('users', 'users', NULL, 0, 0);
      END;
    `);
    console.log('✅ Users table and SyncStatus initialized');
  }

  async getLastSyncDate() {
    const pool = await sql.connect(this.sqlConfig);
    const result = await pool.request().query(`
      SELECT last_sync_date FROM SyncStatus WHERE entity_name = 'users'
    `);
